...
parser.add_argument("-t", "--target",
                    help="Restricted Page (target)")
parser.add_argument("-cl", "--badcl",
                    help="Restricted Access Content Length")
parser.add_argument("-r", "--range",
					help="IP range i.e. 0.0.0.0-255.255.255.255")
parser.add_argument("-w", "--workers",
					help="Worker/thread count - default is 20", default=20)
...
for ip in tqdm(ip_addresses):
	...
	if response.headers['content-length'] > args.badcl:
		...
		ip_save_file.append(args.target + ": " + x_forwarded_for_header + "\n")
		print "Access granted with {0}".format(ip)
		break
